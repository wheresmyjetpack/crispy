...
describe Ingredients::StoreIngredients do
  ...
  let(:form_instance) { instance_double(Ingredients::IngredientForm, name: nil) }
  ...
  describe '#call' do
    ...
    context 'when there are three ingredients' do
      ...
      describe 'the form' do
        subject { form_type }
        it { is_expected.to have_received(:from_params).once.times.with(1) }
        it { is_expected.to have_received(:from_params).once.times.with(2) }
        it { is_expected.to have_received(:from_params).once.times.with(3) }
      end

      describe 'the relation' do
        subject { relation }
        it { is_expected.to have_received(:new).exactly(3).times.with({name: nil}) }
      end

      describe 'the new records from the relation' do
        subject { record }
